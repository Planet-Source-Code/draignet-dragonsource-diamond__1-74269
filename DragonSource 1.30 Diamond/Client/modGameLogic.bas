...
Dim Ending As String
    ...
    If UCase(Dir(App.Path & "\GUI", vbDirectory)) <> "GUI" Then
        Call MsgBox("You are missing the required GUI files!", vbOKOnly, GAME_NAME & " Problem")
        End
    End If
    
    If Not AllGUIFilesExist Then
        Call MsgBox("You are missing some GUI files that must be in the GUI folder!", vbOKOnly, GAME_NAME & " Problem")
        End
    Else
        frmSendGetData.Picture = LoadPicture(App.Path & "\GUI\loading.gif")
        frmSendGetData.Show
    End If

    Call SetStatus("Initializing the client's properties...")
    DoEvents
    ...
    PLAYERWORNEQ2_CHAR = Chr$(197)
    PLAYERINVSLOTS_CHAR = Chr$(198)
    ...
    Call SetStatus("Setting up DirectX...")
    DoEvents
    ...
    If FileExist("config.ini") Then
        frmMain.chkbubblebar.Value = Val(GetVar(FileName, "CONFIG", "SpeechBubbles"))
        frmMain.chkEmoSound.Value = Val(GetVar(FileName, "CONFIG", "EmoticonSound"))
        frmMain.chknpcname.Value = Val(GetVar(FileName, "CONFIG", "NPCName"))
        frmMain.chkplayername.Value = Val(GetVar(FileName, "CONFIG", "PlayerName"))
        frmMain.chkplayerdamage.Value = Val(GetVar(FileName, "CONFIG", "NPCDamage"))
        frmMain.chknpcdamage.Value = Val(GetVar(FileName, "CONFIG", "PlayerDamage"))
        frmMain.chkmusic.Value = Val(GetVar(FileName, "CONFIG", "Music"))
        frmMain.chkSound.Value = Val(GetVar(FileName, "CONFIG", "Sound"))
        frmMain.chkAutoScroll.Value = Val(GetVar(FileName, "CONFIG", "AutoScroll"))
    ...
    frmMain.scrlMusicVol.Value = MusicVolume
    frmMain.lblMusicVol.Caption = MusicVolume
    ...
    frmMain.txtChat.BackColor = RGB(R1, G1, B1)
    ...
    frmMain.txtMyTextBox.BackColor = RGB(R1, G1, B1)
    ...
    frmMain.Picture9.BackColor = RGB(R1, G1, B1)
    frmMain.Picture8.BackColor = RGB(R1, G1, B1)
    frmMain.picInv3.BackColor = RGB(R1, G1, B1)
    frmMain.itmDesc.BackColor = RGB(R1, G1, B1)
    frmMain.picWhosOnline.BackColor = RGB(R1, G1, B1)
    frmMain.picGuildAdmin.BackColor = RGB(R1, G1, B1)
    frmMain.picGuild.BackColor = RGB(R1, G1, B1)
    frmMain.picEquip.BackColor = RGB(R1, G1, B1)
    frmMain.picPlayerSpells.BackColor = RGB(R1, G1, B1)
    frmMain.picOptions.BackColor = RGB(R1, G1, B1)
    
    frmMain.chkbubblebar.BackColor = RGB(R1, G1, B1)
    frmMain.chkEmoSound.BackColor = RGB(R1, G1, B1)
    frmMain.chknpcname.BackColor = RGB(R1, G1, B1)
    frmMain.chkplayername.BackColor = RGB(R1, G1, B1)
    frmMain.chkplayerdamage.BackColor = RGB(R1, G1, B1)
    frmMain.chknpcdamage.BackColor = RGB(R1, G1, B1)
    frmMain.chkmusic.BackColor = RGB(R1, G1, B1)
    frmMain.chkSound.BackColor = RGB(R1, G1, B1)
    frmMain.chkAutoScroll.BackColor = RGB(R1, G1, B1)
    ...
    frmMain.lstSpells.BackColor = RGB(R1, G1, B1)
    ...
    frmMain.lstOnline.BackColor = RGB(R1, G1, B1)
    ...
    frmMain.lstFriend.BackColor = RGB(R1, G1, B1)
...
Sub SetStatus(ByVal Caption As String)
    If Not frmSendGetData.Visible Then frmSendGetData.Visible = True
...
Dim rec_back As RECT
    ...
    If GettingMap = False Then
        If GetPlayerPOINTS(MyIndex) > 0 Then
            frmMain.AddStr.Visible = True
            frmMain.AddDef.Visible = True
            frmMain.AddSpeed.Visible = True
            frmMain.AddMagi.Visible = True
        Else
            frmMain.AddStr.Visible = False
            frmMain.AddDef.Visible = False
            frmMain.AddSpeed.Visible = False
            frmMain.AddMagi.Visible = False
        End If
        ' Visual Inventory
        Dim Q As Long
        Dim Qq As Long
...
End If
    ...
    If GettingMap = False Then
        If ScreenMode = 0 Then
            ...
            'MyText = frmMain.txtMyTextBox.Text
            ...
            For I = 1 To MAX_BLT_LINE
                If BattlePMsg(I).Index > 0 Then
                    If BattlePMsg(I).Time + 7000 > GetTickCount Then
                        Call DrawText(TexthDC, 1 + sx, BattlePMsg(I).y + frmMain.picScreen.Height - 15 + sx, Trim$(BattlePMsg(I).Msg), QBColor(BattlePMsg(I).Color))
                    Else
                        BattlePMsg(I).done = 0
                    End If
                End If
                ...
                If BattleMMsg(I).Index > 0 Then
                    If BattleMMsg(I).Time + 7000 > GetTickCount Then
                        Call DrawText(TexthDC, (frmMain.picScreen.Width - (Len(BattleMMsg(I).Msg) * 8)) + sx, BattleMMsg(I).y + frmMain.picScreen.Height - 15 + sx, Trim$(BattleMMsg(I).Msg), QBColor(BattleMMsg(I).Color))
                    Else
                        BattleMMsg(I).done = 0
                    End If
                End If
    ...
    End If
        ...
        If InEditor Then
            frmMain.shpSelect.Left = MouseX
            frmMain.shpSelect.Top = MouseY
        End If
        ...
        'Lock the FPS so it won't go above 30ish (about 32)
...
' The next two Subs PDBltPlayer and PDBltPlayerTop are used for optional PaperDoll (PD)

Sub PDBltPlayer(ByVal Index As Long)
Dim Anim As Byte
Dim x As Long, y As Long
Dim AttackSpeed As Long
...
Sub PDBltPlayerTop(ByVal Index As Long)
Dim Anim As Byte
Dim x As Long, y As Long
Dim AttackSpeed As Long
...
MyText = frmMain.txtMyTextBox.Text
    ...
    If (KeyAscii = vbKeyReturn) Then
        ...
        If frmMain.txtMyTextBox.Text = vbNullString Then
            Call SendData(RETURNSCRIPT_CHAR & END_CHAR)
            Exit Sub
        End If
    
        frmMain.txtMyTextBox.Text = vbNullString
            ...
            If Len(MyText) > 6 Then
                ...
                If IsNumeric(MyText) = True Then
                    If MyText > -1 And MyText < 101 Then
                        ...
                        frmMain.scrlMusicVol.Value = MyText
                        frmMain.lblMusicVol.Caption = MyText
    ...
    If (KeyAscii <> vbKeyReturn) And (KeyAscii <> vbKeyBack) Then
        ...
        If KeyAscii >= 32 And KeyAscii <= 126 Then
            'frmMain.txtMyTextBox.Text = frmMain.txtMyTextBox.Text & Chr(KeyAscii)
