...
Sub TcpInit()
    ...
    frmMain.Socket.RemoteHost = GetVar(FileName, "IPCONFIG", "IP")
    frmMain.Socket.RemotePort = Val(GetVar(FileName, "IPCONFIG", "PORT"))
    LoadAllGUIPictures
    frmMain.Picture = LoadPicture(App.Path & "\GUI\game.gif")
End Sub
...
Dim z As Long
    ...
    If Parse$(0) = MAXINFO_CHAR Then
        ...
        SCROLLING_ON = Val(Parse$(13))
        PAPERDOLL_ON = Val(Parse$(14))
    ...
    If Parse$(0) = PLAYERWORNEQ2_CHAR Then
        z = Val(Parse$(1))
        If z <= 0 Then Exit Sub
    ...
    If Parse$(0) = PLAYERINVSLOTS_CHAR Then
    ...
    If Parse$(0) = PLAYERPOINTS_CHAR Then
        ...
        frmMain.lblPoints.Caption = Val(Parse$(1)) & " points"
        Exit Sub
    End If
    ...
    If Parse$(0) = PLAYERHP_CHAR Then
        ...
        If GetPlayerMaxHP(MyIndex) > 0 Then
            ...
            frmMain.shpHP.Width = (((GetPlayerHP(MyIndex) / frmMain.lblHP.Width) / (GetPlayerMaxHP(MyIndex) / frmMain.lblHP.Width)) * frmMain.lblHP.Width)
    ...
    If Parse$(0) = PLAYERMP_CHAR Then
        ...
        If GetPlayerMaxMP(MyIndex) > 0 Then
            ...
            frmMain.shpMP.Width = (((GetPlayerMP(MyIndex) / frmMain.lblMP.Width) / (GetPlayerMaxMP(MyIndex) / frmMain.lblMP.Width)) * frmMain.lblMP.Width)
    ...
    If (Parse$(0) = PLAYERSTATSPACKET_CHAR) Then
        ...
        If SubStr > 0 Then
            frmMain.lblSTR.Caption = "Strength - " & Val(Parse$(1)) - SubStr & " (+" & SubStr & ")"
        Else
            frmMain.lblSTR.Caption = "Strength - " & Val(Parse$(1))
        End If
        If SubDef > 0 Then
            frmMain.lblDEF.Caption = "Defense - " & Val(Parse$(2)) - SubDef & " (+" & SubDef & ")"
        Else
            frmMain.lblDEF.Caption = "Defense - " & Val(Parse$(2))
        End If
        If SubMagi > 0 Then
            frmMain.lblMAGI.Caption = "Magic - " & Val(Parse$(4)) - SubMagi & " (+" & SubMagi & ")"
        Else
            frmMain.lblMAGI.Caption = "Magic - " & Val(Parse$(4))
        End If
        If SubSpeed > 0 Then
            frmMain.lblSPEED.Caption = "Speed - " & Val(Parse$(3)) - SubSpeed & " (+" & SubSpeed & ")"
        Else
            frmMain.lblSPEED.Caption = "Speed - " & Val(Parse$(3))
        End If
        frmMain.lblEXP.Caption = Val(Parse$(6)) & " / " & Val(Parse$(5))
        
        frmMain.shpTNL.Width = (((Val(Parse$(6)) / frmMain.lblEXP.Width) / (Val(Parse$(5)) / frmMain.lblEXP.Width)) * frmMain.lblEXP.Width)
        frmMain.lblLevel.Caption = "Level " & Val(Parse$(7))
    ...
    If Parse$(0) = STARTSPEECH_CHAR Then
        ...
        frmTalk.Show vbModeless, frmMain
...
' :::::::::::::::::::::::::::::
    ...
    Dim Sound As String
    Dim Sound_ID As Variant
    
        Sound_ID = Parse$(1)
        
        If IsNumeric(Sound_ID) = False Then
            If Right(Sound_ID, 4) = ".wav" Then
                Call PlaySound(Sound_ID)
            Else
    ...
    If Parse$(0) = FRIENDLIST_CHAR Then
        ...
        n = 1
        frmMain.lstFriend.AddItem "Online:"
        Do While Parse$(n) <> NEXT_CHAR
            If Trim$(Parse$(n)) <> vbNullString Then frmMain.lstFriend.AddItem Parse$(n)
            n = n + 1
        Loop
        
        frmMain.lstFriend.AddItem " "
        frmMain.lstFriend.AddItem "All friends:"
        
        n = n + 1
        
        Do While Parse$(n) <> NEXT_CHAR
            frmMain.lstFriend.AddItem Parse$(n)
            n = n + 1
        Loop
        Exit Sub
    End If
